...
[`RegistryItem`](/sdk/api/getModelByName#registryitem) ` | undefined` — The matching model constant, or `undefined` if not found. See [`getModelByName()`](/sdk/api/getModelByName#registryitem) for the full `RegistryItem` shape.
