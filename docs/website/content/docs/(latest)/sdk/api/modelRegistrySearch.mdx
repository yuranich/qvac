...
`Promise<`[`ModelRegistryEntry[]`](/sdk/api/modelRegistryGetModel#modelregistryentry)`>` — Matching model entries. See [`modelRegistryGetModel()`](/sdk/api/modelRegistryGetModel#modelregistryentry) for the `ModelRegistryEntry` shape.
