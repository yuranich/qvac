...
<Callout type="info">
**Important:** for the exact config fields supported by each model type (key), see [`modelConfig` — `loadModel()` at `@qvac/sdk` API reference](/sdk/api#loadmodel).
</Callout>
...
| `config` | Model config overrides (same as [`modelConfig` in `loadModel()`](/sdk/api#loadmodel)). | `object` | No |
