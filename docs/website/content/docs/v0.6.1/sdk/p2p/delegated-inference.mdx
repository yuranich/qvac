...
Delegation is configured at model-load time by passing a `delegate` object to [`loadModel()`](/v0.6.1/sdk/api/loadModel). The provider is started separately using [`startQVACProvider()`](/v0.6.1/sdk/api/startQVACProvider).

## Functions

**Provider:**
1. [`startQVACProvider()`](/v0.6.1/sdk/api/startQVACProvider) — join a topic and serve requests
2. [`stopQVACProvider()`](/v0.6.1/sdk/api/stopQVACProvider) — leave the topic

**Consumer:**
1. [`loadModel()`](/v0.6.1/sdk/api/loadModel) — with `delegate` option
2. [`completion()`](/v0.6.1/sdk/api/completion) / [`transcribe()`](/v0.6.1/sdk/api/transcribe) / [`translate()`](/v0.6.1/sdk/api/translate) / etc. — same as local
...
- To stop a running provider and leave the topic, call [`stopQVACProvider()`](/v0.6.1/sdk/api/stopQVACProvider).
