...
When cache exists, only the last message is sent to the model (includes multimodal attachments). Use [`deleteCache()`](/v0.6.1/sdk/api/deleteCache) to remove cached sessions.
