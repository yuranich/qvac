...
export function buildBuiltinSpecifier(
  sdkName: string,
  suffix: string,
): string {
...
export function parseBuiltinSpecifier(
  specifier: string,
  sdkName: string,
...
export function resolvePluginSpecifiers(
  config: QvacConfig,
  sdkName: string,
  logger: Logger,
): string[] {
  ...
  if (plugins.length === 0) {
    ...
    logger.info(
      "   For smaller bundles, add a 'plugins' array to qvac.config.*\n",
    );
    plugins = allBuiltins;
  }
