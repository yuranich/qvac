#!/usr/bin/env bash
#
# Create a local Python venv at ./venv with the packages the EasyOCR
# .pth -> .gguf converter needs (see scripts/requirements.txt).
...
#
# After provisioning, the converter is invoked via scripts/convert-model.sh
# (or `npm run convert-model`), which auto-discovers
# ./venv/{bin,Scripts}/python before falling back to the system python3.
...
case "$base_platform" in
  mingw*|cygwin*|msys*)
    ...
    echo "         npm run setup:venv -- --python \"\$(py -3 -c 'import sys; print(sys.executable)')\"" >&2
...
echo "  ./scripts/convert-model.sh ~/.EasyOCR/model/english_g2.pth models/english_g2.gguf"
echo "(or:   npm run convert-model -- ~/.EasyOCR/model/english_g2.pth models/english_g2.gguf)"
