...
// min_size in python - applied AFTER merging (like ExecutorTorch's
// removeSmallBoxesFromArray)
...
// Maximum different in box height. Boxes with very different text size should
// not be merged.
constexpr float MAX_BOX_HEIGHT_DEVIATION = 0.5F;

// width_ths in python
// Maximum horizontal distance to merge boxes.
constexpr float MAX_BOX_HORIZONTAL_MERGE_DISTANCE =
    0.5F; // Match EasyOCR default (was 1.0F)

// Maximum width for merged boxes to prevent excessive compression in
// recognizer. Based on recognizer input 512x64, with 15% margin like
// ExecutorTorch. kMaxWidth = kLargeRecognizerWidth + (kLargeRecognizerWidth *
// 0.15) = 512 + 76.8 = 588
 ...
 */
/**
 * @brief modifies the order of points in box so they are in clockwise order,
 * starting with the top-left most point
 *
...
/**
 * @brief transforms a polygon that may be in any orientation into a
 * horizontally or vertically aligned box if it is already close to being
 * aligned
 *
 * @param box : box to be populated with the results
 * @param poly : generic polygon to be transformed
 */
void alignDiamondShape(
    std::array<cv::Point2f, 4>& box, std::vector<cv::Point>& segMapPoints) {
  ...
  if (std::abs(1.0 - boxRatio) <= DIAMOND_RATIO_TOL /* NOLINT(readability-magic-numbers,cppcoreguidelines-avoid-magic-numbers) */) {
    ...
    box.at(0) = cv::Point2f(
        static_cast<float>(minXIt->x), static_cast<float>(minYIt->y));
    box.at(1) = cv::Point2f(
        static_cast<float>(maxXIt->x), static_cast<float>(minYIt->y));
    box.at(2) = cv::Point2f(
        static_cast<float>(maxXIt->x), static_cast<float>(maxYIt->y));
    box.at(3) = cv::Point2f(
        static_cast<float>(minXIt->x), static_cast<float>(maxYIt->y));
  } else {
...
void StepBoundingBox::loadConnectedComponents(
  ...
  cv::Mat centroids;
  nLabels_ = cv::connectedComponentsWithStats(
      textScoreComb8U, labels_, stats_, centroids, 4);
}
...
StepBoundingBox::Output StepBoundingBox::process(StepBoundingBox::Input input) {
  ...
  QLOG(
      ...
      "[BoundingBox] nLabels=" + std::to_string(nLabels_) +
          ", textMap size=" + std::to_string(input.textMap.cols) + "x" +
          std::to_string(input.textMap.rows) +
          ", linkMap size=" + std::to_string(input.linkMap.cols) + "x" +
  ...
  // ExecutorTorch approach: extract all components first (with low area
  // threshold), merge them, then filter small boxes after merging. This allows
  // small words like "or" to be merged with adjacent text before filtering.
  std::vector<std::array<cv::Point2f, 4>> listOfBoxes;
  for (int i = 1; i < nLabels_; i++) {
    // Use low area threshold to allow small components to be extracted and
    // merged
  ...
  QLOG(
      qvac_lib_inference_addon_cpp::logger::Priority::DEBUG,
      "[BoundingBox] Found " + std::to_string(listOfBoxes.size()) +
          " raw boxes from components");

  auto [alignedBoxes, unalignedBoxes] =
      turnPolysIntoBoxes(listOfBoxes, input.context.boxMarginMultiplier);
  QLOG(
      qvac_lib_inference_addon_cpp::logger::Priority::DEBUG,
      "[BoundingBox] After turnPolysIntoBoxes: " +
          std::to_string(alignedBoxes.size()) + " aligned, " +
          std::to_string(unalignedBoxes.size()) + " unaligned");

  std::vector<std::array<float, 4>> mergedAlignedBoxes =
      groupAndMergeAlignedBoxes(
          alignedBoxes, input.context.boxMarginMultiplier);
  QLOG(
      qvac_lib_inference_addon_cpp::logger::Priority::DEBUG,
      "[BoundingBox] After merge: " +
          std::to_string(mergedAlignedBoxes.size()) + " merged aligned boxes");

  std::vector<AlignedBox> outputAlignedBoxes =
      getOutputAlignedBoxes(input.imgResizeRatio, mergedAlignedBoxes);
  std::vector<UnalignedBox> outputUnalignedBoxes =
      getOutputUnalignedBoxes(input.imgResizeRatio, unalignedBoxes);

  QLOG(
      qvac_lib_inference_addon_cpp::logger::Priority::DEBUG,
      "[BoundingBox] Final output: " +
          std::to_string(outputAlignedBoxes.size()) + " aligned, " +
          std::to_string(outputUnalignedBoxes.size()) +
...
StepBoundingBox::getBoxFromComponent(Input& input, int component) {
...
StepBoundingBox::createSegmentationMap(cv::Size imgSize, int component) {
  ...
  const int niter = static_cast<const int>(
      sqrt(area * std::min(width, height) / (width * height)) * 2);
  ...
  cv::Mat kernel =
      cv::getStructuringElement(cv::MORPH_RECT, {1 + niter, 1 + niter});
...
StepBoundingBox::turnPolysIntoBoxes(
  ...
  for (const auto& poly : polys) {
    const float slopeUp = (poly[1].y - poly[0].y) /
                          std::max(MIN_DELTA_FOR_SLOPE, poly[1].x - poly[0].x);
    const float slopeDown =
        (poly[2].y - poly[3].y) /
        std::max(MIN_DELTA_FOR_SLOPE, poly[2].x - poly[3].x);
    ...
    if (std::max(std::abs(slopeUp), std::abs(slopeDown)) < SLOPE_THRESHOLD) {
      ...
      const auto heightVal = static_cast<float>(std::sqrt(
          std::pow(poly[3].x - poly[0].x, 2) +
          std::pow(poly[3].y - poly[0].y, 2)));
      const auto widthVal = static_cast<float>(std::sqrt(
          std::pow(poly[1].x - poly[0].x, 2) +
          std::pow(poly[1].y - poly[0].y, 2)));
      ...
      unalignedBoxes.push_back(
          {cv::Point2f(xOne, yOne),
           cv::Point2f(xTwo, yTwo),
           cv::Point2f(xThree, yThree),
           cv::Point2f(xFour, yFour)});
    }
  }
...
std::vector<std::vector<std::array<float, ALIGNED_META_SIZE>>>
StepBoundingBox::getListOfBoxesToMerge(
  ...
  for (const auto& box : alignedBoxes) {
    if (currentGroup.empty()) {
      ...
      if (std::abs(meanBoxYCenter - box[IDX_Y_CENTER]) <
          Y_CENTER_MAX_DEVIATION * meanBoxHeight) {
        currentGroup.push_back(box);
        sumBoxHeight += box[IDX_BOX_HEIGHT];
        sumBoxYCenter += box[IDX_Y_CENTER];
        count++;
...
std::vector<std::array<float, 4>> StepBoundingBox::groupAndMergeAlignedBoxes(
    const std::vector<std::array<float, ALIGNED_META_SIZE>>& alignedBoxes,
    float boxMarginMultiplier) {
  ...
  for (auto& boxes : combinedList) {
    if (boxes.size() == 1) {
      ...
      const int margin = static_cast<int>(
          boxMarginMultiplier * std::min(box[1] - box[0], box[IDX_BOX_HEIGHT]));
      mergedList.push_back(
          {box[0] - static_cast<float>(margin),
           box[1] + static_cast<float>(margin),
      ...
      for (const auto& box : boxes) {
        if (currentGroup.empty()) {
          ...
          const float meanGroupHeight =
              sumGroupHeight / static_cast<float>(groupCount);
          bool heightSimilar =
              (std::abs(meanGroupHeight - box[IDX_BOX_HEIGHT]) <
               MAX_BOX_HEIGHT_DEVIATION * meanGroupHeight);
          bool horizontallyClose =
              ((box[0] - currentXMax) <
               MAX_BOX_HORIZONTAL_MERGE_DISTANCE * (box[3] - box[2]));

          // Check if merged width would exceed max (ExecutorTorch approach)
          float mergedWidth =
              box[1] -
              currentGroup[0][0]; // xMax of new box - xMin of first box
      ...
      for (const auto& group : mergedGroups) {
        if (group.size() == 1) {
          ...
          const int margin =
              static_cast<int>(boxMarginMultiplier * std::min(width, height));
          mergedList.push_back(
              {box[0] - static_cast<float>(margin),
               box[1] + static_cast<float>(margin),
               box[2] - static_cast<float>(margin),
          ...
          const float boxHeight = yMax - yMin;
          const int margin = static_cast<int>(
              boxMarginMultiplier * std::min(boxWidth, boxHeight));
          mergedList.push_back(
              {xMin - static_cast<float>(margin),
               xMax + static_cast<float>(margin),
               yMin - static_cast<float>(margin),
               yMax + static_cast<float>(margin)});
        }
      }
    }
  }
...
std::vector<AlignedBox> StepBoundingBox::getOutputAlignedBoxes(
    const float imgResizeRatio,
  ...
  QLOG(
      qvac_lib_inference_addon_cpp::logger::Priority::DEBUG,
      "[getOutputAlignedBoxes] Processing " +
          std::to_string(mergedList.size()) +
          " boxes, linkMapBinary size=" + std::to_string(linkMapBinary_.cols) +
          "x" + std::to_string(linkMapBinary_.rows));
  ...
  for (const auto& box : mergedList) {
    ...
    if (std::max(width, height) > MIN_SIZE) {
      ...
      if (roiWidth <= 0 || roiHeight <= 0) {
        ...
        QLOG(
            qvac_lib_inference_addon_cpp::logger::Priority::DEBUG,
            "[getOutputAlignedBoxes] Skipped box: invalid ROI after clamp "
            "(roiX=" +
                std::to_string(roiX) + ", roiY=" + std::to_string(roiY) +
                ", roiW=" + std::to_string(roiWidth) +
  ...
  QLOG(
      qvac_lib_inference_addon_cpp::logger::Priority::DEBUG,
      "[getOutputAlignedBoxes] Result: " + std::to_string(filtered.size()) +
          " filtered, skipped " + std::to_string(skippedMinSize) +
          " (min size), " + std::to_string(skippedInvalidRoi) +
          " (invalid ROI)");

  return filtered;
}

std::vector<UnalignedBox> StepBoundingBox::getOutputUnalignedBoxes(
    const float imgResizeRatio,
    ...
    if (std::max(diffX, diffY) > MIN_SIZE) {
      ...
      for (const auto& point : poly) {
        ...
        int px = std::max(
            0,
            std::min(
                linkMapBinary_.cols - 1, cvRound(point.x / imgResizeRatio)));
        int py = std::max(
            0,
            std::min(
                linkMapBinary_.rows - 1, cvRound(point.y / imgResizeRatio)));
